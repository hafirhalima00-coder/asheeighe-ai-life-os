...
export class OpenAIProvider implements AIProvider {
  ...
  async chat(messages: ChatMessage[], options?: ChatCompletionOptions): Promise<ChatCompletionResponse> {
    ...
    if (!response.ok) {
      ...
      throw new ExternalServiceError('OpenAI', `Request failed: ${response.status} ${body}`);
    }
    ...
    if (!choice) throw new ExternalServiceError('OpenAI', 'No completion returned');
  ...
  async chatStream(messages: ChatMessage[], options?: ChatCompletionOptions): Promise<ReadableStream> {
    ...
    if (!response.ok) {
      const body = await response.text();
      throw new ExternalServiceError('OpenAI', `Stream request failed: ${response.status} ${body}`);
    }
